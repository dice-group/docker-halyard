...
run-sdk:
	docker run -it --rm --network hbase --env-file ./hbase.env earthquakesan/halyard-sdk:1.0.0-halyard1.2 /bin/bash
...
add-rdf:
	...
	docker run -it --rm --network hbase --volume $(shell pwd)/rdf:/rdf --env-file ./hadoop.env --env-file ./hbase.env earthquakesan/halyard-sdk:1.0.0-halyard1.2 bulkload /foaf.rdf /tmp halyardtable

build-webapps:
	docker build -t earthquakesan/halyard-webapps:1.0.0-halyard1.2 ./webapps

run-webapps:
	docker service create --name halyard --label traefik.docker.network=hbase --label traefik.port=8080 --network hbase --env-file ./webapps.env --publish 8081:8080 earthquakesan/halyard-webapps:1.0.0-halyard1.2
...
load-data-10:
	...
	docker run --name benchmark-load-10 --network hbase --env-file ./webapps.env earthquakesan/halyard-sdk:1.0.0-halyard1.2 bulkload /benchmark_10.nt /tmp benchmark10
...
load-data-50:
	...
	docker run -it --rm --network hbase --env CORE_CONF_fs_defaultFS=hdfs://namenode:9000 bde2020/hadoop-base:1.2.0-hadoop2.7.4-java8 hadoop fs -rm -f -r /tmp
	docker run -d --name benchmark-load-50 --network hbase --env-file ./webapps.env earthquakesan/halyard-sdk:1.0.0-halyard1.2 bulkload /benchmark_50.nt /tmp benchmark50
...
load-data-100:
	...
	docker run -it --rm --network hbase --env CORE_CONF_fs_defaultFS=hdfs://namenode:9000 bde2020/hadoop-base:1.2.0-hadoop2.7.4-java8 hadoop fs -rm -f -r /tmp
	docker run -d --name benchmark-load-100 --network hbase --env-file ./webapps.env earthquakesan/halyard-sdk:1.0.0-halyard1.2 bulkload /benchmark_100.nt /tmp benchmark100

continue-data-load:
	docker run -it --name continue-data-load --network hbase --env-file ./webapps.env  earthquakesan/halyard-sdk:1.0.0-halyard1.2 hbase org.apache.hadoop.hbase.tool.LoadIncrementalHFiles hdfs://namenode:9000/tmp benchmark100 
